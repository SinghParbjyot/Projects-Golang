...
func Test_should_return_error_when_transaction_type_is_no_deposit_or_withdrawl(t *testing.T) {
	//Arrange
...
func Test_should_return_error_when_amount_is_less_than_Zero(t *testing.T) {
	...
	if appError.Message != "Amount cannot be less than zero" {
		t.Error("Inavalid message while validation amount")
	}
	if appError.Code != http.StatusUnprocessableEntity {
		t.Error("Inavalid code while validation amount")
	}
}
